...
def platform_self_audit_readonly_final(inp: "OrionRuntimeIn", *, org: str = "public") -> Dict[str, Any]:
    ...
    try:
        payload.update(_build_dispatch_receipt_payload(
            message=inp.message,
            visible_agent="orion",
            selected_specialists=list(payload.get("selected_specialists") or []),
            dispatch_executed=bool(payload.get("dispatch_executed")),
            fallback_used=bool(payload.get("fallback_used")),
            fallback_reason=str(payload.get("fallback_reason") or "final_readonly"),
        ))
    except Exception:
        pass
...
def _looks_like_team_roster_question(message: str) -> bool:
    raw = str(message or "").strip().lower()
    if not raw:
        return False
    return bool(re.search(r"quem\s+est[aá]\s+no\s+time|quais\s+agentes|quantos\s+agentes|team\s+roster|roster|lista\s+de\s+agentes", raw, flags=re.IGNORECASE))


def _looks_like_presence_status_question(message: str) -> bool:
    raw = str(message or "").strip().lower()
    if not raw:
        return False
    return bool(re.search(r"est[aá]s?\s+online|esta\s+online|est[aá]\s+online|is\s+online|voc[eê]\s+est[aá]\s+online", raw, flags=re.IGNORECASE))
...
_ORCHESTRATION_OPERATIONAL_VERBS = [
    ...
    "pergunte", "mande", "teste se", "dispatch", "orchestrate", "ask", "ping",
    "status ok", "estás online", "esta online", "está online", "is online",
]


def _looks_like_direct_agent_message_request(message: str) -> bool:
    txt = str(message or "").strip().lower()
    if not txt:
        return False
...
def _looks_like_orchestrator_dispatch_request(message: str) -> bool:
    txt = str(message or "").strip().lower()
    if not txt:
        return False
    ...
    *,
    ...
    return {
        "intent": (
            ...
            if _looks_like_direct_agent_message_request(message)
            else "orchestrator_dispatch_readonly"
            if _looks_like_orchestrator_dispatch_request(message)
            else ""
        ),
        "visible_agent": str(visible_agent or "").strip(),
        "target_agent": target_agent,
        "selected_specialists": selected,
        "dispatch_attempted": bool(dispatch_attempted),
        "dispatch_executed": bool(dispatch_executed),
        "dispatch_receipt_id": (f"dispatch_{_new_id()[:12]}" if dispatch_attempted else None),
        "fallback_used": bool(fallback_used),
        "fallback_reason": str(fallback_reason or "").strip(),
    }


def _apply_dispatch_receipt_payload(payload: Dict[str, Any], receipt: Dict[str, Any]) -> Dict[str, Any]:
    out = dict(payload or {})
    out.update(dict(receipt or {}))
    return out
